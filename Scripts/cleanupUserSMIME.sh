...
# Login Hash List
loginhashlist=$(/usr/bin/security find-certificate -a -m -Z /Users/"$currentUser"/Library/Keychains/login.keychain-db | grep -E 'SHA-1|email' | awk '/DOMAIN.COM/ {print x}; { x=$NF}')

# User Hash List
userhashlist=$(/usr/bin/security find-certificate -c "$cn" -a -Z /Users/"$currentUser"/Library/Keychains/login.keychain-db | grep SHA-1 | awk '{print $NF}')
...
if [[ ! -z "$uniquelist" ]]; then
	while read -r line
	do
		/usr/bin/security delete-certificate -Z $line /Users/"$currentUser"/Library/Keychains/login.keychain-db
	done <<< "$uniquelist"
fi