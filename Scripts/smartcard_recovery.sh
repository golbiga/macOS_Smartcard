...
bootVolumeName=$(/usr/sbin/diskutil apfs listGroups | /usr/bin/awk -F: '/(System)/ { getline; gsub(/^[ \t]+/,"",$2); print $2}')
...
userExempt=$(/usr/bin/defaults read "/Volumes/$bootVolumeName/var/db/dslocal/nodes/Default/users/$uid" SmartCardEnforcement 2>/dev/null | /usr/bin/awk 'NR==2' | /usr/bin/sed 's/^[ \t]*//')
...
/usr/bin/defaults write "/Volumes/$bootVolumeName/var/db/dslocal/nodes/Default/users/$uid" SmartCardEnforcement -array-add 2
...
local launch_daemon_path="/Volumes/$bootVolumeName/Library/LaunchDaemons/$launch_daemon".plist
